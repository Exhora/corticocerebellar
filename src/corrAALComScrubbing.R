...
for(ind in grupoTeste){
	...
	file<-paste0(paste0("../original/pm.ORIGINAL.00", phenotype$SUB_ID[ind], sep=""),".cleanEPI_aal_TCs.1D", sep="")
	print(ind)
...
for(i in 1:length(roisCerebro)){
	pval[i] <- summary(lm(corRois[1:p,i] ~ diagnotico + funcIdade[grupo] + phenotype$SEX[grupo] + factor(phenotype$SITE_ID[grupo])))$coeff[68]
